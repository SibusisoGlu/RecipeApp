...
protocol FoodListRetreivable {
    func fetchFoodDetails(for query: String, completion: @escaping ([FoodModel]?) -> Void)
}
...
//    var cache = [String: [FoodModel]] = [:]
    ...
    private func convertFoodModel(foodData: Food?) -> [FoodModel]? {
        guard let foodInformation = foodData?.results else { return nil }
        var foodResults: [FoodModel] = []
        for foodItem in foodInformation {
            let food = FoodModel(id: foodItem.id ?? 0,
